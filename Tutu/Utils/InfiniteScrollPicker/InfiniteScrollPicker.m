...
- (void)initInfiniteScrollViewWithSelectedItem:(int)index
{
    ...
    if (_imageAry.count > 0)
    {
        ...
        self.contentSize = CGSizeMake(_imageAry.count * 2 * _itemSize.width, self.frame.size.height);
        if (self.contentSize.width < self.mj_width * 2) {
            self.contentSize = CGSizeMake(self.mj_width * 2, self.mj_height);
        }
