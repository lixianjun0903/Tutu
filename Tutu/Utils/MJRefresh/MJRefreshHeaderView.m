...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    {
       // myLabel.text = [object valueForKey:MJRefreshContentOffset];
        CGPoint  offset = [[object valueForKey:MJRefreshContentOffset]CGPointValue ];
        CGFloat offsetY = offset.y;
        if (offsetY < -64) {
            if (self.beginRefreshingTaget) {
                UIViewController *controller=(UIViewController *)self.beginRefreshingTaget;
                if(controller==nil){
                    WSLog(@"这是啥");
                }
...
- (void)playSound{
    ...
    NSURL *filePath   = [[NSBundle mainBundle] URLForResource:@"refresh" withExtension: @"m4a"];
    AudioServicesCreateSystemSoundID((__bridge CFURLRef)filePath, &soundID2);
