...
@property (nonatomic, copy) void (^progressBlock)(uint64_t receivedLength, uint64_t totalLength, NSInteger remainingTime, float progress,NSString *identification);
...
- (instancetype)initWithURL:(NSURL *)url identification:(NSString *)identification
               downloadPath:(NSString *)pathToDL
...
- (void)connection:(NSURLConnection*)connection didReceiveData:(NSData *)data
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.progressBlock) {
            self.progressBlock(self.receivedDataLength, self.expectedDataLength, self.remainingTime, self.progress,self.identification);
        }
...
- (void)notifyFromCompletionWithError:(NSError *)error pathToFile:(NSString *)pathToFile
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            [[CacheManager sharedCacheManager] clearUnValidVieo];
        });
...
- (NSString *)fileName
{
    //WSLog(@"%@",[[NSURL URLWithString:[self.downloadURL absoluteString]] lastPathComponent]);
    NSString *name = [[NSURL URLWithString:[self.downloadURL absoluteString]] lastPathComponent];
    NSArray *components = [name componentsSeparatedByString:@"."];
    NSString *tempName = [NSString stringWithFormat:@"%@.temp",[components firstObject]];
    return _fileName ? _fileName : tempName;
}
