...
-(void)addHotspotHandler
{
    ...
    [self setOnTap:^(CGPoint pt) {
        ...
        if(attributes!=nil){
            NSRange range= NSMakeRange([attributes[@"range.location"] integerValue], [attributes[@"range.length"] integerValue]);
            
            NSString *clickText=[weakSelf.text substringWithRange:range];
            WPAttributedStyleAction* actionStyle = attributes[@"dict"][@"WPAttributedStyleAction"];
            if (actionStyle) {
                actionStyle.action(clickText);
            }
        }
    }];
}
...
-(NSMutableDictionary *)textAttributesAtPoint:(CGPoint)pt
{
        ...
        if(pt.y < (floor(lineOrigin.y) + floor(descent))) {
            ...
            for (CFIndex run=0; run<runCount; run++) {
                ...
                if (i >= range.location && i<= range.location+range.length) {
                    ...
                    [dictionary setValue:[NSString stringWithFormat:@"%ld",range.location] forKey:@"range.location"];
                    [dictionary setValue:[NSString stringWithFormat:@"%ld",range.length] forKey:@"range.length"];
                    NSDictionary *dict = (__bridge NSDictionary*)CTRunGetAttributes(glyphRun);
                    [dictionary setValue:dict forKey:@"dict"];
                    break;
                }
            }
            if (dictionary) {
